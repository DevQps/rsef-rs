...
pub fn download(registry: Registry, timestamp: i64) -> Result<Box<dyn Read>, Box<std::error::Error>>
{
    ...
    match registry {
        ...
        Registry::APNIC => {
            let url = format!("https://ftp.apnic.net/stats/apnic/{}/delegated-apnic-extended-{}{}{}.gz", year, year, month, day);
            println!("{}", url);
            return Ok(Box::new(Decoder::new(reqwest::get(url.as_str())?)?));
        },
        Registry::ARIN => {
            // TODO: Works only for dates after 2017.
            let url = format!("https://ftp.arin.net/pub/stats/arin/delegated-arin-extended-{}{}{}", year, month, day);
            println!("{}", url);
            return Ok(Box::new(reqwest::get(url.as_str())?));
        },
        Registry::LACNIC => {
            let url = format!("https://ftp.lacnic.net/pub/stats/lacnic/delegated-lacnic-extended-{}{}{}", year, month, day);
            println!("{}", url);
            return Ok(Box::new(reqwest::get(url.as_str())?));
        },
        Registry::RIPE => {
            let url = format!("https://ftp.ripe.net/pub/stats/ripencc/{}/delegated-ripencc-extended-{}{}{}.bz2", year, year, month, day);
            println!("{}", url);
    ...
    pub fn read_all(&mut self) -> Result<Vec<Line>, std::io::Error> {
        ...
        loop {
            ...
            lines.push(Line::Record(Record {
                ...
                id: if fields.len() > 7 { fields[7].to_string() } else { "".to_string() },
            }));
        }
